...
async def run_in_docker(
    ...
    """Run a single org phase inside a Docker container.

    The container runs ``uv run magelab <config> --output-dir /app`` with
    the output_dir mounted at ``/app``.

    For API_KEY auth, the key is passed via ``-e ANTHROPIC_API_KEY=...``.
    For SUB auth, credentials are already staged in the mounted output_dir
    (no Docker flags needed — the orchestrator inside the container fans
    them out into agent session dirs).

    Called by the pipeline execution module. Not part of the public API.
    ...
    Args:
        ...
        resume_mode: Resume mode passed to the containerized magelab CLI.
...
def run_in_workspace(
    ...
    Checks for a ``.docker_image`` marker in output_dir. If present, runs the
    command inside a Docker container with the workspace mounted. Otherwise
    ...
    if docker_marker.exists():
        image = docker_marker.read_text().strip()
        docker_cmd = [
            "docker",
            "run",
            "--rm",
            "-v",
            f"{workspace_dir.resolve()}:/app/workspace",
            "-w",
            "/app/workspace",
        ]
        if sys.platform != "win32":
            docker_cmd += ["--user", f"{os.getuid()}:{os.getgid()}"]
        # Set PYTHONPATH so imports work inside the container
        docker_cmd += ["-e", "PYTHONPATH=/app/workspace"]
        if auth is not None and auth.mode == AuthMode.API_KEY and auth.api_key:
            docker_cmd += ["-e", f"ANTHROPIC_API_KEY={auth.api_key}"]
        docker_cmd += [image, "uv", "run", "--project", "/opt/magelab", "--no-sync"] + cmd
